...
from meridianalgo.quant.market_microstructure import (
    ...
    MarketImpactModel, TickDataAnalyzer, calculate_microprice
)
...
from meridianalgo.quant.high_frequency import (
    ...
    HFTSignalGenerator, MicropriceEstimator, OrderBook, OrderBookLevel
)
...
class TestExecutionAlgorithms:
    ...
    def test_vwap(self):
        ...
        historical_volume = pd.Series(np.random.randint(100, 1000, len(dates)), index=dates)
...
class TestFactorModels:
    ...
    def test_fama_french_model(self):
        ...
        assert all(factor in results['coefficients'] for factor in ['Market', 'Size', 'Value'])
...
class TestRegimeDetection:
    ...
    def test_market_state_classifier(self):
        ...
        assert all(state in ['low_volatility', 'normal', 'high_volatility'] 
