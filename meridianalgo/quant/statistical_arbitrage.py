...
class PairsTrading:
    ...
    def calculate_hedge_ratio(self, series1: pd.Series, series2: pd.Series,
    ...
    def generate_signals(self, series1: pd.Series, series2: pd.Series,
    ...
    def variance_ratio_test(returns: pd.Series, lags: List[int] = [2, 5, 10]) -> Dict:
        ...
        for k in lags:
            ...
            results[f'VR({k})'] = {
                ...
                'pvalue': pvalue,
                'interpretation': 'mean_reversion' if vr < 1 else ('momentum' if vr > 1 else 'random_walk')
            }
...
class SpreadAnalyzer:
    ...
    def calculate_spread_statistics(spread: pd.Series, window: int = 252) -> Dict:
        ...
        return {
            ...
            'current_zscore': (spread.iloc[-1] - spread.mean()) / spread.std() if spread.std() != 0 else 0
        }
    
    @staticmethod
    def bollinger_bands_on_spread(spread: pd.Series, window: int = 20,
    ...
    @staticmethod
    def optimal_entry_exit_thresholds(spread: pd.Series, 
        ...
        def calculate_sharpe(thresholds):
            ...
            for i in range(1, len(zscore)):
                ...
                elif abs(zscore.iloc[i]) < exit_threshold or np.sign(zscore.iloc[i]) != np.sign(zscore.iloc[i-1]):
                    position = 0
                
                if position != 0:
                    ret = position * (spread.iloc[i] - spread.iloc[i-1]) - transaction_cost
                    returns.append(ret)
