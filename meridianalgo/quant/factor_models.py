...
class FamaFrenchModel:
    ...
    def fit(self, returns: pd.Series, 
            ...
            risk_free_rate: Optional[pd.Series] = None) -> Dict:
        """
        ...
        if self.model_type == 'three_factor':
            ...
            X = factor_data.loc[common_index, ['MKT', 'SMB', 'HML', 'RMW', 'CMA']].values
        ...
        # Alpha t-stat
        alpha_var = mse * (1/len(y) + np.mean(X, axis=0) @ np.linalg.inv(X.T @ X) @ np.mean(X, axis=0).T)
...
class APTModel:
    ...
    def extract_factors(self, return_matrix: pd.DataFrame) -> pd.DataFrame:
        ...
        """
        # Standardize returns
        standardized_returns = (return_matrix - return_matrix.mean()) / return_matrix.std()
    ...
    def fit_asset(self, asset_returns: pd.Series) -> Dict:
        ...
        return {
            ...
            'betas': betas,
            'r_squared': r_squared,
            'predicted_return': alpha + np.sum(list(betas.values()) * self.factors.iloc[-1].values)
        }
...
class CustomFactorModel:
    ...
    def fit(self, returns: pd.DataFrame, factors: pd.DataFrame,
        ...
        for asset in returns.columns:
            ...
            ir = alpha / np.std(residuals) * np.sqrt(252) if np.std(residuals) > 0 else 0
    ...
    def calculate_factor_contributions(self, asset: str, 
...
class FactorRiskDecomposition:
    ...
    @staticmethod
    def decompose_variance(weights: np.ndarray, 
                          factor_exposures: np.ndarray,
                          factor_covariance: np.ndarray,
                          specific_variance: np.ndarray) -> Dict:
        """
        ...
        for i in range(factor_exposures.shape[1]):
            ...
            factor_contributions[f'Factor{i+1}'] = var_i / total_variance if total_variance > 0 else 0
        ...
        return {
            ...
            'factor_risk_pct': factor_variance / total_variance if total_variance > 0 else 0,
            'specific_risk_pct': specific_risk_variance / total_variance if total_variance > 0 else 0,
            'factor_contributions': factor_contributions
        }
    
    @staticmethod
    def marginal_factor_contributions(weights: np.ndarray,
                                     factor_exposures: np.ndarray,
        ...
        total_risk = np.sqrt(portfolio_exposures @ factor_covariance @ portfolio_exposures.T)
        pct_contributions = (weights * marginal_contribs) / total_risk if total_risk > 0 else np.zeros_like(weights)
...
class AlphaCapture:
    ...
    @staticmethod
    def calculate_pure_alpha(returns:pd.Series, 
                            factor_returns: pd.DataFrame,
                            factor_loadings: Dict[str, float]) -> pd.Series:
        """
        Calculate pure alpha by removing factor contributions.
        
        Alpha = Return - Σ(beta_i * factor_return_i)
        """
    ...
    @staticmethod
    def information_coefficient(predicted_returns: pd.Series,
    ...
    @staticmethod
    def fundamental_factor_scores(price_data: pd.DataFrame,
    ...
    @staticmethod
    def optimize_factor_tilts(expected_factor_returns: Dict[str, float],
                             factor_covariance: np.ndarray,
                             factor_names: List[str],
                             max_tilt: float = 0.5) -> Dict[str, float]:
