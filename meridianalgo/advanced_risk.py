...
class AdvancedVaR:
    ...
    def historical_var(
        ...
        if method == 'simple':
            ...
            decay_factor = 0.94
            weights = np.array([(decay_factor ** i) for i in range(len(portfolio_returns))][::-1])
    ...
    def parametric_var(
        ...
        if distribution == 'normal':
            ...
            # Expected Shortfall
            cvar = portfolio_mean + np.sqrt(portfolio_var) * (norm.pdf(z_score) / self.alpha)
            ...
            if self.alpha < 0.5:
                cvar = (portfolio_mean + np.sqrt(portfolio_var) * scaling_factor * 
                       (t_dist.pdf(t_score, dof_est) * (dof_est + t_score**2) / 
    ...
    def component_var(
        ...
        for i, asset in enumerate(returns.columns):
            ...
            # Component VaR
            component_var_i = portfolio_weights[i] * marginal_var_i * portfolio_var / portfolio_vol
...
class StressTesting:
    ...
    def run_hypothetical_stress_test(
        ...
        for scenario_name, scenario_data in self.hypothetical_scenarios.items():
            ...
            portfolio_shock = sum(correlated_shocks.get(asset, 0) * weight 
    ...
    def factor_stress_test(
        ...
        # Apply factor shocks
        factor_impact = sum(exposure * shock 
    ...
    def reverse_stress_test(
        ...
        if len(loss_periods) == 0:
            return {'found_scenarios': False, 'message': 'No historical scenarios found'}
        ...
        common_assets = [analysis['worst_performing_asset'] for analysis in loss_analysis.values()]
