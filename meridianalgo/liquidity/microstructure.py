...
class MarketMicrostructure:
    ...
    def calculate_pin(
        self,
        daily_trades: Optional[pd.DataFrame] = None,
    ...
    def huang_stoll_decomposition(self) -> Dict[str, float]:
        ...
        if self.quotes is None or self.trades is None:
            return {'adverse_selection': 0.33, 'inventory': 0.33, 'order_processing': 0.33}
        ...
        if 'side' in merged.columns:
            merged['direction'] = merged['side'].map({'buy': 1, 'sell': -1, 'neutral': 0})
        else:
    ...
    def _calculate_realized_spread_internal(self, merged: pd.DataFrame, lag: int = 5) -> float:
        ...
        merged['realized'] = 2 * merged['direction'] * (merged['price'] - merged['future_mid'])
    ...
    def realized_spread(self, lag: int = 5) -> pd.Series:
        ...
        if 'side' in self.trades.columns:
            merged['direction'] = self.trades['side'].map({'buy': 1, 'sell': -1}).fillna(0)
        else:
            merged['mid'] = (merged['bid'] + merged['ask']) / 2
            merged['direction'] = np.sign(merged['price'] - merged['mid'])
...
class OrderFlowAnalyzer:
    ...
    def rolling_imbalance(self) -> pd.Series:
        ...
        buy_vol = (self.trades['size'] * (self.trades['side'] == 'buy').astype(int)).rolling(self.window).sum()
