...
class SpreadAnalyzer:
    ...
    def __init__(
        ...
        self.quotes['spread_bps'] = (self.quotes['quoted_spread'] / self.quotes['mid']) * 10000
    ...
    def average_spread(self) -> float:
        ...
        if 'duration' in self.quotes.columns:
            return (self.quotes['quoted_spread'] * self.quotes['duration']).sum() / self.quotes['duration'].sum()
...
class RealizedSpread:
    ...
    def _calculate(self):
        ...
        merged['realized_spread'] = 2 * merged['direction'] * (merged['price'] - merged['future_mid'])
...
class EffectiveSpread:
    ...
    def _calculate(self):
        ...
        merged['effective_spread_bps'] = (merged['effective_spread'] / merged['mid']) * 10000
        
        # Price improvement
        merged['price_improvement'] = merged['quoted_spread'] / 2 - abs(merged['price'] - merged['mid'])
    ...
    def by_size(self, n_buckets: int = 5) -> pd.DataFrame:
        ...
        self.data['size_bucket'] = pd.qcut(
            self.data['size'], n_buckets, labels=range(1, n_buckets + 1), duplicates='drop'
        )
