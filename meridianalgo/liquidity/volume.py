...
class VolumeProfile:
    ...
    def price_profile(
        ...
        self.data['price_bin'] = pd.cut(prices, bins=bins, labels=bin_labels, include_lowest=True)
        ...
        return pd.DataFrame({
            ...
            'volume_pct': (profile.values / total_vol * 100) if total_vol > 0 else 0
        })
    ...
    def value_area(self, pct: float = 0.70) -> Tuple[float, float]:
        ...
        return (
            value_area_profile['price'].min(),
            value_area_profile['price'].max()
        )
    
    def vwap(self) -> float:
        """Calculate Volume Weighted Average Price."""
        return (self.data[self.price_col] * self.data[self.volume_col]).sum() / self.data[self.volume_col].sum()
    ...
    def iceberg_detection(self, child_threshold: float = 0.5) -> pd.DataFrame:
        ...
        for idx, row in trades.iterrows():
            if not current_cluster:
                ...
                size_similar = abs(row['size'] - last['size']) / last['size'] < child_threshold
                ...
                if size_similar and price_same:
                    ...
                    if len(current_cluster) >= 3:  # Minimum cluster size
                        clusters.append({
                            ...
                            'avg_size': np.mean([t['size'] for t in current_cluster]),
...
class VPIN:
    ...
    def _calculate_vpin(self):
        ...
        self.vpin_series = imbalance.rolling(self.n_buckets).sum() / volume.rolling(self.n_buckets).sum()
