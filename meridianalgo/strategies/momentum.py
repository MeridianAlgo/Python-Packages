...
from ..core import calculate_returns, calculate_rsi, calculate_macd
...
class BaseStrategy(ABC):
    ...
    def backtest(self, data: pd.DataFrame, initial_capital: float = 100000) -> Dict[str, pd.Series]:
...
class RSIMeanReversion(BaseStrategy):
    ...
    def generate_signals(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        for asset in data.columns:
            ...
            signals.loc[rsi > self.overbought_threshold, asset] = -1  # Sell when overbought
            signals.loc[(rsi >= self.exit_threshold) & 
                       (rsi <= 100 - self.exit_threshold), asset] = 0  # Exit at neutral
...
class MACDCrossover(BaseStrategy):
    ...
    def generate_signals(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        for asset in data.columns:
            prices = data[asset]
            macd_data = calculate_macd(prices, fast_period=self.fast_period, 
