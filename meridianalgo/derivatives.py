...
class OptionsPricer:
    ...
    def calculate_greeks(
        ...
        # Charm (delta decay)
        if SCIPY_AVAILABLE:
            charm = -np.exp(-q * T) * norm.pdf(d1) * (2 * r * T - d2 * sigma * np.sqrt(T)) / (2 * T * sigma * np.sqrt(T))
        else:
            charm = 0.0
        
        # Speed (gamma change with respect to spot)
        if SCIPY_AVAILABLE:
            speed = -np.exp(-q * T) * norm.pdf(d1) * (1 + d1 / (sigma * np.sqrt(T))) / (S ** 2 * sigma * np.sqrt(T))
        else:
            speed = 0.0
...
class VolatilitySurface:
    ...
    def construct_volatility_surface(
        ...
        if len(implied_vols.shape) == 1:
            # 1D interpolation
            vol_interp = interp1d(maturities, implied_vols, kind='cubic', fill_value='extrapolate')
    ...
    def stochastic_volatility_simulation(
        ...
        for i in range(1, n_steps + 1):
            ...
            v[:, i] = np.maximum(0, v[:, i - 1] + kappa * (theta - v[:, i - 1]) * dt + 
...
class ExoticOptions:
    ...
    def asian_option_geometric(
        ...
        sigma_adj = sigma * np.sqrt((n_observations + 1) * (2 * n_observations + 1) / (6 * n_observations ** 2))
