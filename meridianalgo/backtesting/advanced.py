...
from ..core import calculate_metrics, calculate_max_drawdown
...
class BacktestEngine:
    ...
    def run_backtest(
        ...
        for date, row in data.iterrows():
            ...
            self._process_date(date, row, positions.loc[date] if date in positions.index else None)
    ...
    def _process_date(self, date: datetime, market_data: pd.Series, target_positions: Optional[pd.Series]):
    ...
    def _calculate_results(self) -> Dict[str, Any]:
        ...
        if self.benchmark_returns is not None:
            benchmark_returns_aligned = self.benchmark_returns.reindex(dates, method='ffill')
    ...
    def _calculate_profit_factor(self) -> float:
        ...
        gross_profit = sum(t.total_cost for t in self.trades if t.action == 'sell' and t.total_cost < 0)
        gross_loss = sum(t.total_cost for t in self.trades if t.action == 'sell' and t.total_cost > 0)
    ...
    def _calculate_turnover(self) -> float:
        ...
        avg_portfolio_value = np.mean([s.portfolio_value for s in self.portfolio_history])
...
class MultiStrategyBacktester:
    ...
    def get_best_strategy(self, metric: str = 'sharpe_ratio') -> Tuple[str, Dict[str, Any]]:
        """Get the best performing strategy by metric."""
        best_name = max(self.results.keys(), key=lambda x: self.results[x]['metrics'].get(metric, -float('inf')))
