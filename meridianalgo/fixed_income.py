...
class BondPricer:
    ...
    def price_bond(
        ...
        duration = self._calculate_macaulay_duration(
            face_value, coupon_rate, yield_to_maturity, years_to_maturity, frequency, price
        )
        ...
        return {
            ...
            'yield_spread': yield_to_maturity - self._get_risk_free_rate(years_to_maturity)
        }
    ...
    def calculate_yield_to_maturity(
        ...
        """
        def price_diff(ytm):
            result = self.price_bond(face_value, coupon_rate, ytm, years_to_maturity, frequency)
    ...
    def _day_count_actual_actual(self, start_date: datetime, end_date: datetime) -> float:
...
class YieldCurveModel:
    ...
    def bootstrap_yield_curve(
        ...
        for i, (price, cf_times, cf_amounts) in enumerate(zip(bond_prices, cash_flow_times, cash_flows)):
            for j, time in enumerate(cf_times):
                time_idx = all_times.index(time)
                
                # Calculate present value of known cash flows
                pv_known = 0.0
                for k, (cf_time, cf_amount) in enumerate(zip(cf_times[:j], cf_amounts[:j])):
                    if cf_time < time:
...
class CreditRiskModel:
    ...
    def merton_model(
        ...
        if not SCIPY_AVAILABLE:
            return self._merton_approximation(asset_value, asset_volatility, debt_face_value, 
                                             risk_free_rate, time_to_maturity, dividend_yield)
        
        d1 = (np.log(asset_value / debt_face_value) + 
              (risk_free_rate - dividend_yield + 0.5 * asset_volatility ** 2) * time_to_maturity) / \
    ...
    def _merton_approximation(self, asset_value, asset_volatility, debt_face_value, 
                              risk_free_rate, time_to_maturity, dividend_yield):
    ...
    def mortgage_backed_security(
        ...
        for month in range(1, n_months + 1):
            ...
            if remaining_principal > 0:
                ...
                if month <= wam:
                    payment = remaining_principal * monthly_rate / (1 - (1 + monthly_rate) ** -(wam - month + 1))
                else:
                ...
                cash_flows[month-1] = interest_payment + principal_payment + prepayment_amount
        ...
        return {
            ...
            'weighted_average_life': np.sum(np.arange(1, n_months + 1) * total_principal) / np.sum(total_principal)
        }
