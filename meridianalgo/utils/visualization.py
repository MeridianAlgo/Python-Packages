...
class PortfolioVisualizer:
    ...
    def plot_risk_return_scatter(
        ...
        # Create scatter plot
        scatter = ax.scatter(annual_vols, annual_returns, 
                            s=100, alpha=0.7, c=range(len(annual_returns)), 
                            cmap='viridis')
        
        # Add labels for each point
        for i, (vol, ret) in enumerate(zip(annual_vols, annual_returns)):
            ax.annotate(returns_data.columns[i], (vol, ret), 
...
class TechnicalAnalysisVisualizer:
    ...
    def plot_price_with_indicators(
        ...
        fig, axes = plt.subplots(n_indicators + 1, 1, figsize=(self.figsize[0], 
                                                             self.figsize[1] * (n_indicators + 1) / 2))
        
        # Plot price
        axes[0].plot(prices.index, prices.values, label='Price', linewidth=2, color='blue')
...
class RiskVisualizer:
    ...
    def plot_returns_distribution(
        ...
        ax1.axvline(returns.mean(), color='red', linestyle='--', linewidth=2, label=f'Mean: {returns.mean():.3f}')
        ax1.axvline(returns.median(), color='green', linestyle='--', linewidth=2, label=f'Median: {returns.median():.3f}')
    ...
    def plot_rolling_risk_metrics(
        ...
        ax1.plot(rolling_vol.index, rolling_vol.values, label=f'{window}-day Rolling Volatility', color='orange')
    ...
    def plot_risk_contribution(
        ...
        bars = ax.bar(assets, contributions, color=sns.color_palette("viridis", len(assets)))
