...
def calculate_hurst_exponent(time_series: Union[pd.Series, np.ndarray], max_lag: int = 20) -> float:
    """Calculate the Hurst exponent of a time series."""
    # Convert to numpy array if it's a pandas Series
    ts = time_series.values if hasattr(time_series, 'values') else np.asarray(time_series)
    
    lags = range(2, max_lag + 1)
    tau = [np.std(np.subtract(ts[lag:], ts[:-lag])) 
           for lag in lags]
...
def rolling_volatility(returns: pd.Series, window: int = 21, annualized: bool = True) -> pd.Series:
