...
class RiskAnalyzer:
        ...
        for date, is_dd in in_drawdown.items():
            if is_dd and current_start is None:
                ...
                duration.append({
                    'start': current_start,
                    'end': date,
                    'duration': (date - current_start).days if hasattr(date - current_start, 'days') else 1,
    ...
    def stress_test(
        ...
        for scenario_name, market_shock in scenarios.items():
            portfolio_impact = sum(betas.get(col, 1) * market_shock 
                                   for col in self.returns.columns) / len(self.returns.columns)
