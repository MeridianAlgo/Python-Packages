...
class PerformanceAnalyzer:
    ...
    def __init__(
        ...
        self.benchmark = self._validate_returns(benchmark) if benchmark is not None else None
    ...
    def _validate_returns(self, returns: Union[pd.Series, pd.DataFrame]) -> pd.Series:
        ...
        if (returns.abs() > 1).any():
            import warnings
            warnings.warn("Returns contain values > 100%, verify data is in decimal format")
    ...
    def alpha(self) -> Optional[float]:
        ...
        alpha_daily = (strategy_returns.mean() - self._daily_rf - 
    ...
    def top_drawdowns(self, n: int = 5) -> pd.DataFrame:
        ...
        for start, end in zip(starts, ends):
            ...
            drawdowns.append({
                ...
                'end': end,
                'depth': depth,
                'duration_days': (end - start).days if hasattr(end - start, 'days') else len(period_dd),
                'recovery_days': (end - trough_date).days if hasattr(end - trough_date, 'days') else 0
            })
    ...
    def rolling_sharpe(self, window: int = 63) -> pd.Series:
        ...
        rolling_vol = self.returns.rolling(window).std() * np.sqrt(self.periods_per_year)
    ...
    def rolling_sortino(self, window: int = 63) -> pd.Series:
        ...
        def _sortino(returns):
            ...
            downside_vol = downside.std() * np.sqrt(self.periods_per_year) if len(downside) > 0 else 0
