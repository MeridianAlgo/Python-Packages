...
class BrinsonAttribution:
    ...
    def allocation_effect(self) -> pd.Series:
        ...
        return (weight_diff * (self.bench_returns - self.bench_returns.mean())).sum(axis=1)
    ...
    def summary(self) -> pd.DataFrame:
        ...
        return pd.DataFrame({
            'Effect': [result.allocation_effect, result.selection_effect,
                      result.interaction_effect, result.total_active],
            'Percentage': [
                result.allocation_effect / result.total_active if result.total_active != 0 else 0,
                result.selection_effect / result.total_active if result.total_active != 0 else 0,
                result.interaction_effect / result.total_active if result.total_active != 0 else 0,
                1.0
            ]
...
class FactorAttribution:
    ...
    def calculate(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'alpha_pct': self.alpha * 252 / self.returns.mean() / 252 if self.returns.mean() != 0 else 0,
            'betas': self.betas,
    ...
    def summary(self) -> pd.DataFrame:
        ...
        for factor, beta in result['betas'].items():
            ...
            rows.append((f'{factor} Contribution', result['factor_contributions'][factor]))
