...
class TearSheet:
    ...
    def plot_cumulative_returns(
        ...
        ax.plot(cumulative.index, cumulative.values, 
                color=self.COLORS['strategy'], linewidth=1.5, label='Strategy')
        ...
        text = f'Total Return: {total_ret:.1%}\nSharpe: {sharpe:.2f}\nMax DD: {max_dd:.1%}'
    ...
    def plot_drawdown(self, ax: Optional[Any] = None) -> Any:
        ...
        ax.fill_between(drawdown.index, drawdown.values, 0,
                       color=self.COLORS['drawdown'], alpha=0.4)
        ax.plot(drawdown.index, drawdown.values,
               color=self.COLORS['drawdown'], linewidth=0.5)
    ...
    def plot_rolling_sharpe(
        ...
        ax.plot(rolling_sharpe.index, rolling_sharpe.values,
               color=self.COLORS['strategy'], linewidth=1)
        ax.axhline(0, color='black', linestyle='-', linewidth=0.5)
        ax.axhline(rolling_sharpe.mean(), color=self.COLORS['neutral'],
                  linestyle='--', linewidth=1, label=f'Mean: {rolling_sharpe.mean():.2f}')
    ...
    def plot_rolling_volatility(
        ...
        ax.plot(rolling_vol.index, rolling_vol.values,
               color=self.COLORS['strategy'], linewidth=1)
        ax.axhline(rolling_vol.mean(), color=self.COLORS['neutral'],
                  linestyle='--', linewidth=1, label=f'Mean: {rolling_vol.mean():.1%}')
    ...
    def plot_monthly_returns_heatmap(self, ax: Optional[Any] = None) -> Any:
        ...
        # Create heatmap
        if HAS_SEABORN:
            sns.heatmap(monthly_table, annot=True, fmt='.1%', 
                       center=0, cmap='RdYlGn', ax=ax,
                       vmin=-0.15, vmax=0.15,
                       annot_kws={'size': 8})
        else:
            im = ax.imshow(monthly_table.values, cmap='RdYlGn', aspect='auto',
                          vmin=-0.15, vmax=0.15)
    ...
    def plot_returns_distribution(self, ax: Optional[Any] = None) -> Any:
        ...
        if HAS_SEABORN:
            sns.histplot(self.returns, kde=True, ax=ax, 
                        color=self.COLORS['strategy'], alpha=0.6)
        else:
            ax.hist(self.returns, bins=50, alpha=0.6, 
                   color=self.COLORS['strategy'], edgecolor='white')
        ...
        ax.axvline(var_95, color='orange', linestyle='--', 
                  linewidth=1.5, label=f'VaR 95%: {var_95:.2%}')
        ax.axvline(var_99, color='red', linestyle='--',
                  linewidth=1.5, label=f'VaR 99%: {var_99:.2%}')
    ...
    def create_full_tear_sheet(
        ...
        plt.suptitle('Full Performance Tear Sheet', fontsize=14, fontweight='bold', y=1.01)
    ...
    def get_metrics_summary(self) -> pd.DataFrame:
        ...
        return pd.DataFrame(
            {'Value': all_metrics.values()},
            index=all_metrics.keys()
        )
