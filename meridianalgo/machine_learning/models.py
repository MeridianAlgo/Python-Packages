...
class BaseFinancialModel(ABC):
    ...
    def evaluate(self, X: np.ndarray, y: np.ndarray) -> Dict[str, float]:
        ...
        if len(predictions) > 1:
            ...
            ic = np.corrcoef(predictions, y)[0, 1] if not np.isnan(np.corrcoef(predictions, y)[0, 1]) else 0.0
    ...
    def get_feature_importance(self) -> Optional[Dict[str, float]]:
        ...
        if len(y) > 1:
            ...
            if np.std(y) > 0:  # Avoid division by zero
                hit_rate = np.mean((y > 0) == (predictions > 0))
...
class LSTMModel(BaseFinancialModel):
    ...
    def __init__(self, config: ModelConfig = None):
        ...
        self.hidden_size = config.hyperparameters.get('hidden_size', 64) if config else 64
        ...
        self.sequence_length = config.hyperparameters.get('sequence_length', 20) if config else 20
        ...
        self.learning_rate = config.training_config.get('learning_rate', 0.001) if config else 0.001
...
class TransformerModel(BaseFinancialModel):
    ...
    def __init__(self, config: ModelConfig = None):
        ...
        self.sequence_length = config.hyperparameters.get('sequence_length', 50) if config else 50
...
class TransformerNetwork(nn.Module):
    ...
    def __init__(self, input_size: int, d_model: int, nhead: int, 
                 num_layers: int, dropout: float, sequence_length: int):
...
class ModelFactory:
    ...
    def create_model(model_type: str, config: ModelConfig = None) -> BaseFinancialModel:
        ...
        elif model_type in ['random_forest', 'gradient_boosting', 'linear', 'ridge', 
                           'lasso', 'elastic_net', 'svr', 'xgboost', 'lightgbm']:
            return TraditionalMLModel(model_type, config)
        else:
            raise ValueError(f"Unknown model type: {model_type}")
    
    @staticmethod
    def create_ensemble(model_configs: List[Tuple[str, ModelConfig]], 
...
class ModelTrainer:
    ...
    def train_model(self, model: BaseFinancialModel, X_train: np.ndarray, 
                   y_train: np.ndarray, X_val: np.ndarray = None, 
                   y_val: np.ndarray = None) -> ModelResult:
...
class LSTMModel(BaseFinancialModel):
    ...
    def __init__(self, config: ModelConfig = None):
        super().__init__("LSTM", config)
        self.sequence_length = config.hyperparameters.get('sequence_length', 60) if config else 60
        self.hidden_size = config.hyperparameters.get('hidden_size', 50) if config else 50
        self.num_layers = config.hyperparameters.get('num_layers', 2) if config else 2
        self.dropout = config.hyperparameters.get('dropout', 0.2) if config else 0.2
        self.device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
...
class GRUModel(BaseFinancialModel):
    ...
    def fit(self, X: np.ndarray, y: np.ndarray, **kwargs) -> 'GRUModel':
        ...
        # Create model
        input_size = X.shape[1]
        self.model = GRUNetwork(input_size, self.hidden_size, self.num_layers, self.dropout).to(self.device)
...
class TransformerModel(BaseFinancialModel):
    ...
    def fit(self, X: np.ndarray, y: np.ndarray, **kwargs) -> 'TransformerModel':
        ...
        # Create model
        input_size = X.shape[1]
        self.model = TransformerNetwork(input_size, self.d_model, self.nhead, 
...
class TradingEnvironment:
    ...
    def _get_observation(self):
        ...
        if self.current_step >= len(self.data):
            return np.zeros(self.data.shape[1] + 3)  # +3 for balance, position, portfolio_value
    ...
    def step(self, action):
        ...
        if action == 1 and self.position <= 0:  # Buy
            ...
            if self.position > 0:  # Close long position
                ...
                self.balance += (self.position * current_price) + profit - (self.transaction_cost * current_price)
                self.position = 0
...
class ReinforcementLearningTrader(BaseFinancialModel):
    ...
    def fit(self, X: pd.DataFrame, y: pd.Series = None, **kwargs) -> 'ReinforcementLearningTrader':
        ...
        for episode in range(self.episodes):
            ...
            if episode % 100 == 0:
                ...
                logger.info(f"Episode {episode}, Average Score: {np.mean(scores[-100:]):.4f}")
...
class EnsembleModel(BaseFinancialModel):
    ...
    def __init__(self, models: List[BaseFinancialModel], weights: Optional[List[float]] = None):
        super().__init__("Ensemble")
        self.models = models
        self.weights = weights or [1.0 / len(models)] * len(models)
...
class EnhancedRandomForest(BaseFinancialModel):
    ...
    def __init__(self, config: ModelConfig = None):
        ...
        self.n_estimators = config.hyperparameters.get('n_estimators', 200) if config else 200
        self.max_depth = config.hyperparameters.get('max_depth', 10) if config else 10
        self.min_samples_split = config.hyperparameters.get('min_samples_split', 5) if config else 5
        self.min_samples_leaf = config.hyperparameters.get('min_samples_leaf', 2) if config else 2
...
class XGBoostModel(BaseFinancialModel):
    ...
    def __init__(self, config: ModelConfig = None):
        ...
        self.learning_rate = config.hyperparameters.get('learning_rate', 0.1) if config else 0.1
...
# Model Factory
class ModelFactory:
    """Factory for creating financial ML models."""
    ...
    @staticmethod
    def create_ensemble(model_types: List[str], configs: List[ModelConfig] = None, 
                       weights: List[float] = None) -> EnsembleModel:
        """Create an ensemble model."""
        if configs is None:
            configs = [None] * len(model_types)
        
        models = []
        for model_type, config in zip(model_types, configs):
            models.append(ModelFactory.create_model(model_type, config))
