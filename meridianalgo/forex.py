            ...
            for j, curr2 in enumerate(currencies[i+1:], i+1):
                for k, curr3 in enumerate(currencies[j+1:], j+1):
                    ...
                    if len(available_pairs) >= 3:
                        ...
                        if pair12 in available_pairs and pair23 in available_pairs and pair31 in available_pairs:
            ...
            if len(pair) == 6:
                ...
                if pair in self.analyzer.data.columns:
                    ...
                    if quote_curr == base_currency:
                        ...
                        exposure[quote_curr] = exposure.get(quote_curr, 0) - position * price
                    else:
                        # Need cross rate conversion (simplified)
                        exposure[base_curr] = exposure.get(base_curr, 0) + position * price * 0.5
                        exposure[quote_curr] = exposure.get(quote_curr, 0) - position * 0.5
...
def get_exotic_currency_pairs() -> List[str]:
    ...
    return [
        ...
        'USDSEK', 'USDNOK', 'EURTRY', 'EURZAR',
        'GBPSEK', 'EURNOK', 'AUDCHF', 'CADCHF'
    ]
