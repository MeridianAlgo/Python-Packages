...
class MonteCarloModel:
    ...
    def price_asian_option(self, option: Option, average_type: str = "arithmetic") -> Dict[str, float]:
    ...
    def price_barrier_option(self, option: Option, barrier: float, 
                           barrier_type: str = "up_and_out") -> Dict[str, float]:
...
class OptionPortfolio:
    ...
    def __init__(self):
        self.positions: List[Tuple[Option, float, str]] = []  # (option, quantity, model)
    
    def add_position(self, option: Option, quantity: float, model: str = "black_scholes"):
    ...
    def scenario_analysis(self, spot_changes: List[float], 
                         vol_changes: List[float] = None) -> pd.DataFrame:
...
ass FiniteDifferenceModel:
    ...
    def price_option_explicit(self, option: Option, S_max: float = None) -> Dict[str, float]:
        ...
        if alpha > 0.5:
            warnings.warn("Explicit scheme may be unstable. Consider using implicit method.")
    ...
    def price_option_implicit(self, option: Option, S_max: float = None) -> Dict[str, float]:
...
class ExoticOptions:
    ...
    @staticmethod
    def price_lookback_option(option: Option, lookback_type: str = "floating_strike") -> Dict[str, float]:
    ...
    @staticmethod
    def price_rainbow_option(options: List[Option], correlation: float = 0.5,
                           option_type: str = "max") -> Dict[str, float]:
    ...
    @staticmethod
    def price_compound_option(option: Option, underlying_option: Option) -> Dict[str, float]:
        ...
        for _ in range(n_simulations):
            ...
            S_T1 = S * np.exp((r - 0.5 * sigma**2) * T1 + sigma * np.sqrt(T1) * np.random.normal())
...
class VolatilitySurface:
    ...
    def __init__(self):
        self.data_points: List[Tuple[float, float, float, float]] = []  # (strike, expiry, price, vol)
        self.surface = None
    
    def add_market_data(self, strike: float, expiry: float, market_price: float,
                       underlying_price: float, risk_free_rate: float, 
    ...
    def get_volatility(self, strike: float, expiry: float) -> float:
        ...
        if np.isnan(vol):
            ...
            distances = [(abs(s - strike) + abs(e - expiry), v) 
    ...
    def calculate_skew(self, expiry: float, atm_strike: float) -> Dict[str, float]:
        ...
        strikes = [0.8 * atm_strike, 0.9 * atm_strike, atm_strike, 
                  1.1 * atm_strike, 1.2 * atm_strike]
...
class OptionStrategy:
    ...
    @classmethod
    def create_straddle(cls, underlying_price: float, strike: float, expiry: float,
                       risk_free_rate: float, volatility: float) -> 'OptionStrategy':
        """Create long straddle strategy."""
        
        strategy = cls()
        
        call = Option(underlying_price, strike, expiry, risk_free_rate, volatility, "call")
        put = Option(underlying_price, strike, expiry, risk_free_rate, volatility, "put")
    ...
    @classmethod
    def create_iron_condor(cls, underlying_price: float, strikes: List[float], 
                          expiry: float, risk_free_rate: float, volatility: float) -> 'OptionStrategy':
        ...
        # Buy OTM put
        put1 = Option(underlying_price, strikes[0], expiry, risk_free_rate, volatility, "put")
        strategy.add_leg(put1, 1, "buy")
        
        # Sell ITM put
        put2 = Option(underlying_price, strikes[1], expiry, risk_free_rate, volatility, "put")
        strategy.add_leg(put2, 1, "sell")
        
        # Sell ITM call
        call1 = Option(underlying_price, strikes[2], expiry, risk_free_rate, volatility, "call")
        strategy.add_leg(call1, 1, "sell")
        
        # Buy OTM call
        call2 = Option(underlying_price, strikes[3], expiry, risk_free_rate, volatility, "call")
        strategy.add_leg(call2, 1, "buy")
        
        return strategy

# Utility functions
def calculate_option_parity(call_price: float, put_price: float, spot: float,
                          strike: float, expiry: float, risk_free_rate: float,
                          dividend_yield: float = 0) -> Dict[str, float]:
    """Check put-call parity and calculate synthetic prices."""
    
    # Put-call parity: C - P = S*e^(-q*T) - K*e^(-r*T)
    parity_value = spot * np.exp(-dividend_yield * expiry) - strike * np.exp(-risk_free_rate * expiry)
