    ...
    def generate(self, threshold: float = None) -> pd.DataFrame:
        ...
        for rule in self.rules:
            ...
            if rule['signal_type'] in ['short', 'both']:
                short_score += (~condition_result.astype(bool)).astype(float) * rule['weight']
...
class TechnicalAnalyzer:
    ...
    def get_signals(self) -> pd.DataFrame:
        ...
        # Combined signal
        signal_cols = ['rsi_signal', 'macd_signal', 'bb_signal', 'stoch_signal', 'trend_signal']
    ...
    def summary(self) -> Dict[str, Any]:
        ...
        current['trend'] = 'bullish' if current['sma_20'] > current['sma_50'] else 'bearish'
        current['volatility'] = 'high' if current['atr_pct'] > 0.02 else 'low'
        current['momentum'] = 'positive' if current['macd'] > current['macd_signal'] else 'negative'
