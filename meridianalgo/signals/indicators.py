...
def KAMA(data: pd.Series, period: int = 10, fast: int = 2, slow: int = 30) -> pd.Series:
    ...
    for i in range(period, len(data)):
        kama.iloc[i] = kama.iloc[i-1] + smoothing.iloc[i] * (data.iloc[i] - kama.iloc[i-1])
...
def TSI(data: pd.Series, long: int = 25, short: int = 13, signal: int = 7) -> Tuple[pd.Series, pd.Series]:
    """
    True Strength Index.
...
def Aroon(
    ...
    """
    aroon_up = 100 * (period - high.rolling(period + 1).apply(
        lambda x: period - x.argmax(), raw=True
    )) / period
    
    aroon_down = 100 * (period - low.rolling(period + 1).apply(
