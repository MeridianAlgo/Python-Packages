...
class MeridianAlgoAPI:
    ...
    def get_market_data(
        ...
        **kwargs
        ...
        logger.info(f"Fetching {interval} data for {', '.join(symbols)} from {start_date} to {end_date}")
    ...
    def calculate_risk_metrics(
        ...
        metrics = {
            ...
            'sortino_ratio': calculate_sortino_ratio(returns, risk_free_rate=risk_free_rate),
            'max_drawdown': calculate_max_drawdown(returns),
            'calmar_ratio': calculate_calmar_ratio(returns),
            'var_95': calculate_value_at_risk(returns, confidence_level=confidence_level),
            'cvar_95': calculate_expected_shortfall(returns, confidence_level=confidence_level)
        }
