...
print(
    f"Portfolio: ${portfolio_value / 1e6:.0f}M  [{', '.join(f'{k}:{v:.0%}' for k, v in weights.items())}]"
)
print()
...
for _, row in summary.iterrows():
    ...
    print(
        f"  {name:33}  {row['portfolio_return']:>10.2%}  "
...
for scenario_name in key_scenarios:
    ...
    print(f"\n{label}:")
    print(
        f"  Portfolio Return: {r.portfolio_return:.2%}  P&L: ${r.portfolio_pnl:,.0f}  [{r.severity}]"
    )
    print(f"  Worst asset: {r.worst_asset}  Best asset: {r.best_asset}")
    print("  Asset-level returns:")
    for asset, ret in r.asset_returns.sort_values().items():
        bar = (
            "+" * max(0, int(ret * 200)) if ret >= 0 else "-" * max(0, int(-ret * 200))
        )
...
print(
    f"{'Scenario':25}  {'Eq Shock':>10}  {'Bnd Shock':>10}  {'Return':>10}  {'P&L':>14}"
)
for name, eq, bnd, fx, comm, _gold_s in custom_scenarios:
    ...
    print(
        f"  {name:23}  {eq:>10.2%}  {bnd:>10.2%}  {r.portfolio_return:>10.2%}  ${r.portfolio_pnl:>12,.0f}"
    )
...
print("Find the equity shock that causes target losses:")
print()
...
print("  (Bond/gold hedge reduces required shock magnitude)")
...
stressed = gen.generate(
    n_scenarios=100_000, horizon_days=1, stress_correlation=True, stress_factor=0.60
)

print(f"1-day scenario generation ({100_000:,} scenarios)")
print(f"Portfolio: {dict(zip(assets, port_weights_simple, strict=False))}")
print()
...
ten_day = gen.generate(
    n_scenarios=100_000, horizon_days=10, stress_correlation=True, stress_factor=0.60
)
print("\n10-day 99% VaR (Basel-style):")
ten_day_var = abs(ten_day["var_99"]) * portfolio_value
print(
    f"  Stressed: ${ten_day_var:>12,.0f}  ({abs(ten_day['var_99']):.3%} of portfolio)"
)
...
print("Composition:     60% equity (US+Intl), 30% bonds+TIPS, 10% alternatives")
print()
print("Scenario Stress Results:")
print(
    f"  GFC 2008-09:            {results.get('gfc_2008_2009', type('', (), {'portfolio_return': float('nan')})()).portfolio_return:.2%}"
)
print(
    f"  COVID Mar 2020:         {results.get('covid_crash_march_2020', type('', (), {'portfolio_return': float('nan')})()).portfolio_return:.2%}"
)
print(
    f"  2022 Rate Shock:        {results.get('rate_shock_2022', type('', (), {'portfolio_return': float('nan')})()).portfolio_return:.2%}"
)
print()
print(
    f"Reverse Stress (break-even equity shock for -10% loss): "
    f"{analyzer.reverse_stress_test(-0.10, 'equity'):.2%}"
)
print()
print("Monte Carlo VaR (100k scenarios, 1-day, 99%, stress corr):")
print(
    f"  ${abs(stressed['var_99']) * portfolio_value:,.0f}  ({abs(stressed['var_99']):.3%} of NAV)"
)
