...
def generate_market(
    n: int, daily_drift: float, daily_vol: float, seed: int = 42
) -> pd.Series:
...
print("Initial portfolio:     $1,000,000")
...
print("1-year bear market scenario (shocked with one-day -8% crash)")
print(f"Initial capital: $1,000,000  Floor: {floor_pct:.0%}")
print()
print(
    f"{'Strategy':20}  {'Final Value':>14}  {'Return':>8}  {'Max DD':>8}  {'Floor Breach':>14}"
)
print(
    f"{'CPPI (3x, 80%)':20}  ${result_bear.portfolio_value.iloc[-1]:>12,.0f}  "
...
print(
    f"{'Buy-and-Hold':20}  ${bah_final:>12,.0f}  {bah_return:>8.2%}  {bah_drawdown:>8.2%}  {'N/A':>14}"
)

cash_value = initial_value * (1 + 0.04 / 252) ** 252
print(
    f"{'100% Cash (4%)':20}  ${cash_value:>12,.0f}  {(cash_value / initial_value) - 1:>8.2%}  {'0.00%':>8}  {'N/A':>14}"
)

protected_floor = initial_value * floor_pct
print(f"\nCPPI floor protection: ${protected_floor:,.0f}")
print(
    f"CPPI terminal value:   ${result_bear.portfolio_value.iloc[-1]:,.0f} "
...
print(
    f"{'Multiplier':>12}  {'Floor':>8}  {'Return':>10}  {'Ann Vol':>10}  {'Max DD':>10}  {'Breaches':>10}"
)
for _, row in sensitivity.iterrows():
    print(
        f"  {row['multiplier']:>10.1f}  {row['floor_pct']:>8.0%}  {row['total_return']:>10.2%}  "
...
print("Bull market (yr 1) then bear market (yr 2)")
print(f"Initial: ${initial:,}  Multiplier: 3x  Floor: 80%")
print()
print(
    f"{'Strategy':25}  {'Final Value':>14}  {'Return':>9}  {'Max DD':>9}  {'Final Floor':>14}"
)
print(
    f"  {'CPPI (fixed 80% floor)':23}  ${r_cppi.portfolio_value.iloc[-1]:>12,.0f}  "
    f"{r_cppi.total_return:>9.2%}  {r_cppi.max_drawdown:>9.2%}  "
    f"${r_cppi.floor_value.iloc[-1]:>12,.0f}"
)
print(
    f"  {'TIPP (ratcheting floor)':23}  ${r_tipp.portfolio_value.iloc[-1]:>12,.0f}  "
    f"{r_tipp.total_return:>9.2%}  {r_tipp.max_drawdown:>9.2%}  "
    f"${r_tipp.floor_value.iloc[-1]:>12,.0f}"
)
print(
    f"  {'Buy-and-Hold':23}  ${bah_btb.iloc[-1]:>12,.0f}  "
...
print(
    "providing stronger downside protection than standard CPPI in the subsequent bear market."
)
