...
print("Parameters: mu=8%, sigma=20%, S0=100, T=1yr")
...
print(
    f"95% CI for mean:       (${result.confidence_interval_95[0]:.4f}, "
...
print(
    f"  95% CI:   (${mc_call['confidence_interval'][0]:.4f}, ${mc_call['confidence_interval'][1]:.4f})"
)
print(
    f"  BS Put:   ${bs_put:.4f}  (put-call parity check: "
...
print(f"Portfolio: {dict(zip(asset_names, weights, strict=False))}")
print(f"Correlated simulation: {result_port.n_paths:,} paths")
print()
print("Portfolio 1yr return distribution:")
...
print(
    f"Heston parameters: kappa={heston.kappa}, theta={heston.theta} ({np.sqrt(heston.theta):.0%} vol)"
)
print(f"                   xi={heston.xi}, rho={heston.rho}")
print()
print("Simulated terminal distribution:")
...
print(
    f"  {'Model':12}  {'5th pct':>10}  {'95th pct':>10}  {'IQR':>10}  {'Tail ratio':>12}"
)
...
print(
    f"  {'Heston':12}  ${result_h.percentile_5:>8.4f}  ${result_h.percentile_95:>8.4f}"
    f"  ${heston_iqr:>8.4f}  {'(stoch vol)':>12}"
)
print(
    f"  {'GBM':12}  ${result_gbm_comp.percentile_5:>8.4f}  ${result_gbm_comp.percentile_95:>8.4f}"
...
print(
    f"            lambda={jdm.lam} jumps/yr, mu_J={jdm.mu_jump:.0%}, sigma_J={jdm.sigma_jump:.0%}"
)
print()
print(f"Terminal distribution ({result_j.n_paths:,} paths):")
print(
    f"  Mean:     ${result_j.mean:.4f}  (expected: ${100 * np.exp(jdm.mu * 1.0):.4f})"
)
...
print(
    f"  Jump model has heavier left tail: "
...
print(
    f"CIR parameters: r0={cir.r0:.2%}, kappa={cir.kappa}, theta={cir.theta:.2%}, sigma={cir.sigma}"
)
...
print(
    f"  Min rate: {result_cir.terminal_values.min():.6%}  (CIR guarantees non-negative rates)"
)
...
print("GBM Engine — ATM options (S=100, K=100, T=1yr, r=5%, sigma=20%):")
...
print(
    f"  VaR:      ${var_result['var']:.4f}  ({var_result['var'] / 100:.2%} of portfolio)"
)
...
print(
    f"  Diff vs BS: ${heston_call['price'] - bs_call_check:+.4f}  "
