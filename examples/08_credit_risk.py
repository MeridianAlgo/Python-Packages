...
for name, equity, ev, debt in firms:
    ...
    r = m.calibrate()
    print(
        f"  {name:20s}  DD={r['distance_to_default']:6.3f}  PD={r['default_probability']:.4%}"
    )
...
for h in [0.005, 0.010, 0.020, 0.040, 0.080, 0.150]:
    ...
    r = c.price()
    print(
        f"  {h:12.3%}  {r.fair_spread * 10000:>16.2f}  {r.survival_probability:>14.6f}"
    )
...
print(
    f"Verified spread:  {check * 10000:.2f} bps (roundtrip error: {abs(check - market_spread) * 1e6:.1f} micro-bps)"
)
...
for mat, spread, hazard in zip(
    maturities, market_spreads, hazard_curve.values, strict=False
):
...
portfolio = pd.DataFrame(
    {
        ...
        "rating": ["BBB", "BB", "B", "BBB", "A", "BB", "CCC", "A"],
        "pd": [0.0020, 0.0080, 0.0250, 0.0020, 0.0005, 0.0080, 0.1000, 0.0005],
        "lgd": [0.45, 0.50, 0.60, 0.40, 0.35, 0.50, 0.70, 0.35],
        "ead": [5e6, 3e6, 1e6, 8e6, 10e6, 2e6, 0.5e6, 15e6],
    }
)
...
print(
    portfolio[
        ["obligor", "rating", "pd", "lgd", "ead", "el", "credit_var_99"]
    ].to_string(
        index=False, float_format=lambda x: f"{x:,.0f}" if x > 100 else f"{x:.4f}"
    )
)
...
print(
    f"  Total Expected Loss:   ${result['total_el'] / 1e3:.1f}K ({result['el_rate']:.3%} of EAD)"
)
