...
print("21-day annualized volatility (latest observation):")
...
print("(lower = more efficient estimator)")
...
print("Realized volatility term structure (annualized):")
...
print(
    f"Interpretation: {'contango (normal)' if slope > 0 else 'backwardation (stress)'}"
)
...
total = len(regimes)
print("Regime distribution (21-day rolling vol):")
...
print(
    f"  {'Regime':15s}  {'Ann Return':>12}  {'Ann Vol':>10}  {'Sharpe':>8}  {'Skew':>8}"
)
for regime, row in stats.iterrows():
    print(
        f"  {regime:15s}  {row['annualized_return']:>12.2%}  {row['annualized_vol']:>10.2%}  "
