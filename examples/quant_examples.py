...
from meridianalgo.quant.market_microstructure import (
    OrderFlowImbalance, RealizedVolatility, MarketImpactModel
)
...
from meridianalgo.quant.high_frequency import (
    MarketMaking, HFTSignalGenerator
)
from meridianalgo.quant.factor_models import (
    FamaFrenchModel, FactorRiskDecomposition
)
from meridianalgo.quant.regime_detection import (
    HiddenMarkovModel, MarketStateClassifier
)
...
def example_statistical_arbitrage():
    ...
    stock2 = pd.Series(50 + common_trend * 0.5 + np.random.randn(200) * 1.5, name='STOCK2')
    ...
    print(f"   Current Signal: {current_signal} ({'Long spread' if current_signal > 0 else 'Short spread' if current_signal < 0 else 'Neutral'})")
    ...
    print(f"   Speed of Mean Reversion (θ): {params['theta']:.4f}")
...
def example_optimal_execution():
    ...
    print(f"   Market Impact: ${cost_analysis['permanent_impact'] + cost_analysis['temporary_impact']:,.2f}")
    
    print(f"\n   Optimal Execution Schedule (first 5 intervals):")
    for i in range(min(5, len(trajectory))):
        row = trajectory.iloc[i]
        print(f"   Interval {i}: Trade {row['trades']:.0f} shares, Holdings {row['holdings']:.0f}")
...
def example_high_frequency_trading():
    ...
    print(f"   Spread: ${ask_price - bid_price:.2f} ({(ask_price - bid_price)/mid_price * 10000:.1f} bps)")
    ...
    print(f"   Interpretation: {'High informed trading' if current_toxicity > 0.5 else 'Low informed trading'}")
...
def example_factor_models():
    ...
    for factor, beta in results['coefficients'].items():
        ...
        print(f"   {factor:12s}: β = {beta:7.4f}  (t = {t_stat:6.2f}) {'***' if significant else ''}")
    ...
    print(f"   Factor Risk: {decomp['factor_risk_pct']:.1%} ({decomp['total_volatility'] * decomp['factor_risk_pct']:.2%})")
    print(f"   Specific Risk: {decomp['specific_risk_pct']:.1%} ({decomp['total_volatility'] * decomp['specific_risk_pct']:.2%})")
...
def main():
    ...
    print("╔" + "=" * 78 + "╗")
    print("║" + " " * 78 + "║")
    print("║" + "MeridianAlgo v5.0.0 - Professional Quant Module Examples".center(78) + "║")
    print("║" + " " * 78 + "║")
    print("╚" + "=" * 78 + "╝")
