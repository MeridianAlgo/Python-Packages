...
port_ret, bench_ret = generate_manager(
    n=750, alpha_daily=0.0003, te_daily=0.004, seed=42
)
...
def interpret_ir(ir):
    ...
    if ir > 0.00:
        return "positive alpha, marginal skill"
...
print(
    f"{'Manager':25}  {'Active Ret':>12}  {'TE':>8}  {'IR':>8}  {'Up Cap':>8}  {'Dn Cap':>8}  {'Batting':>8}"
)
for name, alpha, te, seed in scenarios:
    ...
    m = a.active_metrics()
    print(
        f"  {name:23}  {m.active_return:>12.2%}  {m.tracking_error:>8.2%}  "
...
port_long, bench_long = generate_manager(
    n=1260, alpha_daily=0.0002, te_daily=0.004, seed=42
)
...
print("Rolling 252-day Information Ratio (5yr history):")
...
print("  >= 90%: concentrated_active (high conviction, likely high fee)")
print("  60-90%: moderately_active")
print("  20-60%: closet_indexer (index-like returns, active fees)")
print("  < 20%:  index_fund")
...
print(
    f"  Total:             {result.total_allocation + result.total_selection + result.total_interaction:.4%}  "
...
print(
    f"  {'Sector':15}  {'PW':>6}  {'BW':>6}  {'PR':>7}  {'BR':>7}  {'Alloc':>8}  {'Select':>8}  {'Total':>8}"
)
for sector, row in detail.iterrows():
    print(
        f"  {sector:15}  {row['Port Wt']:>6.2%}  {row['Bench Wt']:>6.2%}  "
